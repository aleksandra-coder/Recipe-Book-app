...
$dishName = $Servings = $CookingTime = $rating = $category = $ingredients = $instructions = "";
...
// checking if fields are left empty and if required to fill out, the message is displayed
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["Servings"])) {
        $ServingsErr = "Servings are required";
    } else {
    ...
    if (empty($_POST["CookingTime"])) {
        $CookingTimeErr = "Cooking time is required";
    } else {
...
$insert = "INSERT INTO recipe (userID, RecipeName, Servings, PreparationTime, Ratings, Ingredients, Instructions, Images, DateAdded, TimeAdded)
VALUES ('$userAdding', '$dishName', '$Servings', '$CookingTime', '$rating', '$ingredients', '$instructions', '".$fileName."', CURDATE(), CURTIME())";  


if (($conn->query($insert) === TRUE) && ($_SERVER["REQUEST_METHOD"] == "POST")) {
    header( "location: successfullyadded.php" ); //if the data is successfully added to database after submitting, user will be redirected
    exit;
} else {
   echo "";
}
      ...
      <div class="col-lg-6">
         ...
         <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data"> <!-- returns the filename of the currently executing script and converts special characters to HTML entities -->
            <div class="form-group">
               ...
               <div class="col">
                  ...
                  <div class="col">
                     ...
                     <span class="error">* <?php echo $statusMsg;?></span> <!-- show error message -->
                  </div>
                </div>
            </div>
            <div class="form-group">
               <label>Name of the dish</label><input type="text" class="form-control" name="dishName" id="dishName" value="<?php echo $dishName;?>"> <!-- add the user input to database -->
               <span class="error">* <?php echo $dishNameErr;?></span> <!-- show error message -->
            </div>
            <div class="form-group">
               <div class="row">
                  <div class="col">
                  <label>Number of servings</label><input type="text" class="form-control" name="Servings" value="<?php echo $Servings;?>"> <!-- add the user input to database -->
                  <span class="error">* <?php echo $ServingsErr;?></span> <!-- show error message -->
                  </div>
                  <div class="col">
                  <label>Cooking time (min)</label><input type="text" class="form-control" name="CookingTime" value="<?php echo $CookingTime;?>"> <!-- add the user input to database -->
                  <span class="error">* <?php echo $CookingTimeErr;?></span> <!-- show error message -->
                  </div>
                  <div class="col">
                  <label>Rating</label><select class="form-control"  id="rating" name="rating" value="<?php echo $rating;?>"> <!-- add the user input to database -->
            ...
            <div class="form-group">
               ...
               <textarea class="form-control" name="ingredients" id="ingredients" rows="3" value="<?php echo $ingredients;?>"></textarea> <!-- add the user input to database -->
               <span class="error">* <?php echo $ingredientsErr;?></span> <!-- show error message -->
            </div>
            <div class="form-group">
               <label for="exampleFormControlTextarea1">Instructions</label>
               <textarea class="form-control" name="instructions" id="instructions" rows="3" value="<?php echo $instructions;?>"></textarea> <!-- add the user input to database -->
               <span class="error">* <?php echo $instructionsErr;?></span> <!-- show error message -->
            </div>
